...
import anydbm #DUMMY for py2exe
...
class cMainFrame( wx.Frame ):
    def __init__( self, *args, **kwds ):
        ...
        self.Copy_to_Device = wx.MenuItem(wxglade_tmp_menu, ID_MENU_COPY, _("Copy ROMs to Device"), _("Copy Selected ROMs to Device"), wx.ITEM_NORMAL)
        ...
        self.Run_Startup_Wizard = wx.MenuItem(wxglade_tmp_menu, ID_MENU_WIZARD, _("Run Startup Wizard"), _("Run Startup Wizard"), wx.ITEM_NORMAL)
        ...
        self.Trim_Files = wx.MenuItem(wxglade_tmp_menu, ID_MENU_TRIM_FILES, _("Trim Files"), _("Trim Files"), wx.ITEM_NORMAL)
        ...
        self.Auto_Backup_Saved_Games = wx.MenuItem(wxglade_tmp_menu, ID_MENU_AUTO_BACKUP_SAVES, _("Auto Backup Saved Games"), _("Auto Backup Saved Games"), wx.ITEM_CHECK)
        ...
        wxglade_tmp_menu.AppendSeparator()
        wxglade_tmp_menu.Append(ID_MENU_UPDATE_MASTER_LIST, _("Update &Master List"), _("Update Master List from Advanscene"), wx.ITEM_NORMAL)
        wxglade_tmp_menu.Append(ID_MENU_UPDATE_ROMS, _("Find &New ROMs"), _("Find New ROMs"), wx.ITEM_NORMAL)
        wxglade_tmp_menu.Append(ID_MENU_UPDATE_GFX, _("Update / Check Graphics"), _("Update / Check Graphics"), wx.ITEM_NORMAL)
        wxglade_tmp_menu.AppendSeparator()
        ...
        wxglade_tmp_menu.AppendSeparator()
        self.Hide_Screenshots = wx.MenuItem(wxglade_tmp_menu, ID_MENU_HIDE_SCREENSHOTS, _("Hide Screenshots"), _("Hide Screenshots"), wx.ITEM_CHECK)
        wxglade_tmp_menu.AppendItem(self.Hide_Screenshots)
        self.Hide_Case_Pictures = wx.MenuItem(wxglade_tmp_menu, ID_MENU_HIDE_CASE_PICTURES, _("Hide Case Pictures"), _("Hide Case Pictures"), wx.ITEM_CHECK)
        wxglade_tmp_menu.AppendItem(self.Hide_Case_Pictures)
        self.Hide_Language = wx.MenuItem(wxglade_tmp_menu, ID_MENU_HIDE_LANGUAGE, _("Hide Language"), _("Hide Language"), wx.ITEM_CHECK)
        ...
        self.Hide_Size = wx.MenuItem(wxglade_tmp_menu, ID_MENU_HIDE_SIZE, _("Hide Size"), _("Hide Size"), wx.ITEM_CHECK)
        wxglade_tmp_menu.AppendItem(self.Hide_Size)
        self.Hide_Backup_Save_Games = wx.MenuItem(wxglade_tmp_menu, ID_MENU_HIDE_BACKUP_SAVE_GAMES, _("Hide Backup Save Games"), _("Hide Backup Save Games"), wx.ITEM_CHECK)
        ...
        self.Hide_Save_Game_Type = wx.MenuItem(wxglade_tmp_menu, ID_MENU_HIDE_SAVE_GAME_TYPE, _("Hide Save Game Type"), _("Hide Save Game Type"), wx.ITEM_CHECK)
        ...
        self.Hide_Version = wx.MenuItem(wxglade_tmp_menu, ID_MENU_HIDE_VERSION, _("Hide Version"), _("Hide Version"), wx.ITEM_CHECK)
        ...
        self.SavedGames = wx.StaticText(self.Split_Panel2, -1, _("Backup Saved Games : "))
        self.Tags = wx.StaticText(self.Split_Panel2, -1, _("Tags : "))
        self.Device_Dir_Picker = filebrowse.DirBrowseButton(self.Split_Panel2, -1, changeCallback = self.Device_Dir_Picker_Callback, labelText = _("Device") + " :", startDirectory = Config.Config ["Device_Path"])
...
#            ROMSize_String += Utils.Format_ROM_Size ( ROM.ROM_Size )
        ...
        if Config.Config ["Use_Trimmed"]:
            if ROM.Trimmed:
                ROMSize_String = _( "Official Size : %s, Trimmed Size : %s" ) % ( Utils.Format_ROM_Size( ROM.ROM_Size ), Utils.Format_ROM_Size ( ROM.Effective_Size ) )
            else:
                ROMSize_String = _( "Official Size : %s, Trimmed Size : N/A" ) % Utils.Format_ROM_Size( ROM.ROM_Size )
        else:
            ROMSize_String = _( "ROM Size : " )
            ROMSize_String += Utils.Format_ROM_Size ( ROM.ROM_Size )
        ...
        if ROM.Wifi:
            self.Wifi.SetLabel( _( "Wi-Fi : Yes" ) )
        else:
            self.Wifi.SetLabel( _( "Wi-Fi : No" ) )
...
#        self.Toolbar.AddTool( ID_MENU_SAVE_GAME_MANAGER, eval ( "GFX.getGFX_Icon_SaveGameMgr"+ToolSize+"Bitmap" )(), isToggle=False, shortHelpString=_( "Save Game Manager" ), longHelpString=_( "Save Game Manager" ) )
    ...
    def Populate_Toolbar ( self ):
        ...
        self.Toolbar.AddTool( ID_MENU_TRIM_FILES, eval ( "GFX.getGFX_Icon_Trim"+ToolSize+"Bitmap" )(), isToggle=False, shortHelpString=_( "Trim Files" ), longHelpString=_( "Trim Files" ) )
        self.Toolbar.AddTool( ID_MENU_UNTRIM_FILES, eval ( "GFX.getGFX_Icon_UnTrim"+ToolSize+"Bitmap" )(), isToggle=False, shortHelpString=_( "UnTrim Files" ), longHelpString=_( "UnTrim Files" ) )
        ...
        self.Toolbar.AddTool( ID_TOGGLE_INFO, eval ( "GFX.getGFX_Icon_InfoOn"+ToolSize+"Bitmap" )(), isToggle=True, shortHelpString=_( "Toggle Device Information" ), longHelpString=_( "Toggle Device Information" ) )
        ...
        self.Toolbar.AddTool( ID_TAG_ADD, eval ( "GFX.getGFX_Icon_TagAdd"+ToolSize+"Bitmap" )(), isToggle=False, shortHelpString=_( "Add File to Tag" ), longHelpString=_( "Add File to Tag" ) )
        self.Toolbar.AddTool( ID_TAG_REMOVE, eval ( "GFX.getGFX_Icon_TagRemove"+ToolSize+"Bitmap" )(), isToggle=False, shortHelpString=_( "Remove File from Tag" ), longHelpString=_( "Remove File from Tag" ) )
...
#        dlg.Fit()
    ...
    def Device_Dir_Picker_Callback ( self, event ):
        ...
        if TmpDir != "" and self.In_Init == False and len ( TmpDir ) > 2:
    ...
    def On_Trim_Files( self, event ): # wxGlade: cMainFrame.<event_handler>
        ...
        dlg = cProgressFrame ( self )
        dlg.DisableGuage2()
        dlg.Proccessing_Text.SetLabel ( _( "Analyzing Selected ROMs for Trimmed Sizes" ) )
        dlg.Guage1.SetRange ( To_Process )
        dlg.MakeModal()
        dlg.Center()
        dlg.Show()
    ...
    def On_Run_Startup_Wizard(self, event): # wxGlade: cMainFrame.<event_handler>
        Res = wx.MessageBox( _( "Are you sure you want to Run the Startup Wizard?\n\nThis will Delete All of RToolDS's Settings\nDownloaded Content will be Preserved" ), _( 'Startup Wizard' ), wx.YES_NO| wx.ICON_QUESTION )
        if Res == wx.YES:
    ...
    def On_Device_List_Context_Menu( self, event ): # wxGlade: cMainFrame.<event_handler>
        ...
        if self.Delete_from_Device.IsEnabled():
            Item = wx.MenuItem( Menu, self.Popup_Device_List_Delete, _("Delete from Device") )
            Menu.AppendItem( Item )
        if self.Backup_Save_Game.IsEnabled():
            Item = wx.MenuItem( Menu, self.Popup_Device_List_Delete_Save, _("Delete Save Game") )
            Menu.AppendItem( Item )
    ...
    def On_ROMList_Context_Menu (self, event):
        ...
        Menu = wx.Menu()
        if self.Device_List.IsEnabled():
            Item = wx.MenuItem( Menu, self.Popup_ROMList_Copy, _("Copy ROMs to Device") )
            Menu.AppendItem( Item )
        if self.Trim_Files.IsEnabled():
            Item = wx.MenuItem( Menu, self.Popup_ROMList_Trim, _("Analyze for Trim") )
            Menu.AppendItem( Item )
            Item = wx.MenuItem( Menu, self.Popup_ROMList_UnTrim, _("Reset Trimmed Size") )
            Menu.AppendItem( Item )
        ...
        if self.Import_Save_Game.IsEnabled():
            Item = wx.MenuItem( Menu, self.Popup_ROMList_Import_Save_Game, _("Import Save Game for Selected ROM") )
            Menu.AppendItem( Item )
        if self.Toolbar.GetToolEnabled(ID_TAG_ADD):
            Item = wx.MenuItem( Menu, self.Popup_ROMList_Tag_Add, _("Add Tag to File(s)") )
            Menu.AppendItem( Item )
            if Config.Config ["Last_Tag"] != "":
                Item = wx.MenuItem( Menu, self.Popup_ROMList_Tag_Last, _("Add Tag to File(s) - %s") % (Config.Config ["Last_Tag"]))
                Menu.AppendItem( Item )
        if self.Toolbar.GetToolEnabled(ID_TAG_REMOVE):
            Item = wx.MenuItem( Menu, self.Popup_ROMList_Tag_Remove, _("Remove File(s) from this Tag") )
            Menu.AppendItem( Item )
